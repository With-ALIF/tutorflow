...
export const RoutineCard: React.FC<RoutineCardProps> = ({ routine, students, onEdit, onDelete }) => (
  <div className="relative bg-white dark:bg-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-slate-200 dark:border-slate-700 shadow-sm text-sm">
    <div 
      className="absolute top-0 left-6 right-6 h-1 rounded-b-full opacity-60 bg-indigo-500"
    />
    <div className="flex flex-col gap-4">
      <RoutineCardHeader routine={routine} onEdit={onEdit} onDelete={onDelete} />
      <RoutineCardDetails routine={routine} />
      <RoutineCardStudents className={routine.className} students={students} />
    </div>
  </div>
);
