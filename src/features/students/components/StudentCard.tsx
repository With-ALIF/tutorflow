...
export const StudentCard: React.FC<StudentCardProps> = ({ student, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200/60 dark:border-slate-700 p-5 shadow-sm transition-all group flex flex-col h-full">
      ...
      <div className="flex items-start gap-4 mb-5">
        <Link to={`/students/${student.id}`} className="shrink-0">
          {student.photo ? (
            <img 
              src={student.photo} 
              alt={student.name} 
              className="w-16 h-16 rounded-2xl object-cover ring-4 ring-slate-50 dark:ring-slate-900 transition-transform" 
              referrerPolicy="no-referrer" 
            />
          ) : (
            <div className="w-16 h-16 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-2xl ring-4 ring-slate-50 dark:ring-slate-900 transition-transform">
        ...
        <div className="flex-1 min-w-0">
          <Link to={`/students/${student.id}`}>
            <h3 className="font-bold text-slate-900 dark:text-white transition-colors truncate text-lg">
              {student.name}
            </h3>
          </Link>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[9px] font-bold text-indigo-500">৳{perLecture}/lec</span>
            </div>
          </div>
          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
