...
export const fetchDailyAttendance = async (date: string): Promise<Record<string, 'present' | 'absent'>> => {
...
export const saveAttendance = async (studentId: string, date: string, status: 'present' | 'absent') => {
  if (!auth.currentUser) return;
